---------------------------------------------------------------------------------------------------
-- Script for getting API test reports
...
SELECT	tr.Name,
	...
	WHERE wrj.StartedAtUtc > @fromDate AND wrj.StartedAtUtc < @toDate AND wrj.Name LIKE 'Staging% API %'
...
SELECT	tr.Name,
	...
	JOIN [GHAData].[dbo].[TestResult] tr
	ON tr.WorkflowRunJobId = wrj.Id
	WHERE wrj.StartedAtUtc > @fromDate4weeks AND wrj.Name LIKE 'Staging% API %')
...
--DECLARE @fromDate DATETIME = DATEADD(d, -7, '2024/09/08 14:00:00');
...
--	JOIN [GHAData].[dbo].[TestResult] tr
--	ON tr.WorkflowRunJobId = wrj.Id
--	WHERE wrj.StartedAtUtc > @fromDate AND tr.Result = 'Failed' AND wrj.Name LIKE 'Staging% API%'
